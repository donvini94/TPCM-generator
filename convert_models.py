...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Convert multiple XML models to TPCM format in parallel"
    )
    parser.add_argument(
        "--input-dir",
        "-i",
    ...
    parser.add_argument(
        "--output-dir", "-o", default="input", help="Directory to write TPCM files to"
    )
    parser.add_argument(
        "--pattern",
        "-p",
    ...
    # Set up progress bar
    pbar = tqdm(total=len(xml_files), desc="Converting XML to TPCM", unit="file")
    successful = 0
    failed = 0
    ...
    results = convert_multiple_to_tpcm(
        conversion_paths, args.processes, callback=update_progress
    )
